use clap::{builder::ValueParser, value_parser, Arg, ArgAction, Args, Command, Parser, Subcommand};
...
#[derive(Parser)]
#[command(author = env!("CARGO_PKG_AUTHORS"), version = concat!("version ", env!("SOLANG_VERSION")))]
#[command(
...
pub struct IdlCommand {
    ...
    #[arg(name = "OUTPUT",help = "output file", short = 'o', long = "output", num_args = 1, value_parser =ValueParser::os_string())]
...
pub struct ShellComplete {
    ...
    pub shell_complete: Option<Shell>,
}

impl ShellComplete {
    pub fn command_from_shell(&self) -> Command {
        Command::new("shell-complete").arg(
            Arg::new("SHELL")
...
pub struct Package {
    ...
    #[arg(name = "IMPORTMAP", help = "Map directory to search for solidity files [format: map=path]",value_parser = ValueParser::new(parse_import_map) , action = ArgAction::Append, long = "importmap", short = 'm', num_args = 1)]
    pub import_map: Option<Vec<String>>,
}
...
pub fn imports_arg(package: &Package) -> FileResolver {
    ...
    if let Some(maps) = &package.import_map {
        let map_vec_res: Result<Vec<(String, PathBuf)>, String> =
            maps.iter().map(|f| parse_import_map(f.as_str())).collect();
        let map_vec = match map_vec_res {
            Ok(vec) => vec,
            Err(err) => {
                eprintln!("{err}");
                exit(1);
