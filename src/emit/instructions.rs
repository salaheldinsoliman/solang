...
use crate::emit::expression::{expression, format_print};
...
pub(super) fn process_instruction<'a, T: TargetRuntime<'a> + ?Sized>(
    ...
    match ins {
        ...
        Instr::PopMemory {
            ...
            target.report_error(bin, "pop from empty array".to_string(), Some(*loc), ns);
        ...
        Instr::Print { expr } => {
            ...
            if ns.target.is_substrate() {
                let res = format_print(bin, ns, expr, "print: ".to_string());
                target.print(bin, bin.vector_bytes(res), bin.vector_len(res));
            } else {
        ...
        Instr::DebugPrint { string, loc } => {
            emit_context!(bin);
            target.report_error(bin, string.to_owned(), Some(*loc), ns);
        }
    }
}
