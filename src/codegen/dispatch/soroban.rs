...
pub fn function_dispatch(
    ...
    for cfg in all_cfg.iter_mut() {
        ...
        //wrapper_cfg.params = vec![param.clone()].into();
...
fn encode_args(function: &Function, ns: &Namespace) -> Vec<Expression> {
    ...
    for (i, arg) in function.params.iter().enumerate() {
       ...
       let arg = match arg.ty{
        ...
        ,
         Type::Val => {
            println!("Address  INISSSDEEEE");
            println!("Address  IS {:?}", arg);
            Expression::FunctionArg { loc: arg.loc, ty: Type::Address(false), arg_no: 0 }.cast(&Type::Address(false), ns)
         //Expression::NumberLiteral { loc: pt::Loc::Codegen, ty: Type::Address(false), value: BigInt::from(8_u64) }
         },

        Type::Address(_) => 
            Expression::FunctionArg { loc: arg.loc, ty: arg.ty.clone(), arg_no: i }.cast(&Type::Address(false), ns)
            ,
        _ => unimplemented!(),
